...
let destinationNamesWithin500Kms = travelDestinations.filter(within500).map( t => t.destinationName);
function within500(destination){
    return destination.distanceKms < 500;
}

let destinationNameReachableByFerry = travelDestinations.filter(byFerry).map( t => t.destinationName);

function byFerry(destination){
    return destination.transportations.includes("ferry")
}


let destinationNamesMoreThan300KmsAwayByTrain = travelDestinations.filter(moreThan300KmsAwayByTrain).map( t => t.destinationName);
function moreThan300KmsAwayByTrain(destination){
